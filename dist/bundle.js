...
window.addEventListener('DOMContentLoaded', function () {
  ...
  function sequencer() {
    ...
    Tone.Transport.scheduleRepeat(repeat, '16n'); //effects

    var autoFilter = new Tone.AutoFilter("4n");
    var delay = new Tone.FeedbackDelay("8n");
    var bitCrusher = new Tone.BitCrusher(1); //automation
    //bpm

    document.querySelector('.bpm-wetness').addEventListener('input', function (e) {
      e.preventDefault;
      var bpm = e.currentTarget.value;
    ...
    document.querySelector('.delay-wetness').addEventListener('input', function (e) {
      e.preventDefault;
      var wet = e.currentTarget.value / 100;
      delay.set({
        wet: wet
      });
    }); //bitcrusher
    ...
    document.querySelector('.bitcrush-wetness').addEventListener('input', function (e) {
      e.preventDefault;
      var wet = e.currentTarget.value / 100;
      bitCrusher.set({
        wet: wet
    ...
    var kick = new Tone.Player('../assets/Thumpy.wav').connect(bitCrusher).chain(bitCrusher, delay, autoFilter, Tone.Destination);
    var snare = new Tone.Player('../assets/good-disco-snare_2.wav').connect(bitCrusher).chain(bitCrusher, delay, autoFilter, Tone.Destination);
    var hat = new Tone.Player('../assets/lofi-hat.wav').connect(bitCrusher).chain(bitCrusher, delay, autoFilter, Tone.Destination);
    ...
    var playPauseTrack = function playPauseTrack() {
      if (playing === false) {
  ...
  sequencer(); //do a for loop that generates 8 rows of buttons
